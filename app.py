...
async def index(request: Request):
    return html('欢迎使用极简网络验证Python3版</br>作者：@jiayouzl(modified by @00ffcc)</br>Github：<a href="https://github.com/jiayouzl/python_web_auth" target="_blank">点击访问</a></br>服务器时间：' + verify.get_server_time() + '</br>服务器Python版本：' + sys.version)
...
async def reg(request: Request):
    ...
    if len(machineCode) > 10000:
        return json({'code': 10012, 'msg': '非法的机器码长度'})
    result = verify.reg(machineCode)
    return json(result)
...
async def make_card(request: Request):
    parametes = request.json
    print(parametes)
    result = verify.make_new_card(int(parametes['number']), int(parametes['time']),parametes['category'])
    return json(result)
...
async def user_update(request: Request):
    parametes = request.json
    # print(parametes)
    result = verify.update_user(parametes['machine_code'], int(parametes['expire_date']))
    return json(result)
